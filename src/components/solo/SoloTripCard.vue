<template>
  ...
  >
    <button
      class="cursor-pointer absolute right-3 top-3 rounded-full bg-white/90 px-2 py-1 text-xs text-slate-600 hover:bg-white"
    ...
    <header class="mb-3">
      <div class="flex items-start justify-between gap-1 flex-col">
        <div class="min-w-0 flex justify-center items-center">
          ...
          <h3 class="truncate text-[15px] font-semibold text-slate-900">{{ trip.title }}</h3>
        </div>

        <div
          class="ml-1 mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-slate-500"
        >
    ...
    <div class="flex items-center justify-between">
      ...
      <div
        v-else
        class="flex h-9 w-full items-center justify-between rounded-xl bg-slate-700 px-3 text-[11px] font-medium text-white"
      >
        <span>완료된 여행</span>
      </div>
    </div>
          ...
          <form class="space-y-5" @submit.prevent="handleEditSubmit">
            <section class="flex items-center gap-4">
              ...
              <div class="flex flex-col gap-1 text-[11px] text-slate-500">
                ...
                <p>카드에 보이는 표지 이미지를 바꾸고 싶다면 아래 버튼을 눌러줘.</p>
                <button
                  type="button"
            ...
            <section class="space-y-3">
              <div class="space-y-1.5">
                ...
                <input
                  v-model="editTitle"
                  type="text"
                  class="h-9 w-full rounded-lg border border-slate-200 bg-slate-50/60 px-3 text-sm outline-none transition focus:border-fuchsia-400 focus:bg-white focus:ring-1 focus:ring-fuchsia-100"
                  placeholder="예: 2월 혼자 부산"
                />
              </div>
              ...
              <div class="grid grid-cols-2 gap-2">
                <div class="space-y-1.5">
                  ...
                  <input
                    ...
                    type="date"
                    class="h-9 w-full rounded-lg border border-slate-200 bg-white px-2 text-xs outline-none focus:border-fuchsia-400 focus:ring-1 focus:ring-fuchsia-100"
                  />
                </div>
                <div class="space-y-1.5">
...
const dateLabel = computed(() => {
  const s = props.trip.startDate
  const e = props.trip.endDate
  if (!s && !e) return '날짜 미정'
  if (s && !e) return s
  if (!s && e) return e
  return `${s} ~ ${e}`
